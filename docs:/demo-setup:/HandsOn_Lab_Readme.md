...
# P6 — Prompt 6: Automated Code Change & Pull Request *(~10 min)*

Ask Bob to make a precise, scoped code change directly in the GitHub repository via MCP — adding validation checks to the withdraw route — then open a pull request for review.

**Output:** New branch, committed change and pull request

## Prompt Text

```text
Using the GitHub MCP connection, make the following exact change to my repository bob-banking-app on my GitHub account.
...
2. Find the file in the repository that contains the withdraw route — it handles POST requests for withdrawing funds from a bank account.

3. In that file, locate where the form amount is read from the request, and add these exact three validation checks immediately after that line, before any service or database call is made:

   - If the amount field is empty, return an error:
     "Amount is required"

   - If the amount is not a positive number, return an error:
     "Amount must be greater than zero"

   - If the amount exceeds the current balance, return an error:
     "Insufficient funds"
...
5. Commit with message:
   ...
   "Add validation checks to withdraw route"

6. Create a pull request with:

   - base branch: main
   - compare branch: feature/withdraw-validation
   - title: "Add input validation to withdraw route"
   - body: "Adds three validation checks to the withdraw route to prevent invalid transactions in the banking application."

7. Confirm the PR number and URL.
```

## Covers

- ✓ New feature branch created
- ✓ Three validation checks added to the withdraw route
- ✓ Change committed with a clear message
- ✓ Pull request opened with title, body and branches specified
- ✓ PR number and URL confirmed

> **Tip:** Review the diff carefully before merging — this step edits code directly.

---

# P7 — Prompt 7: Automated Pull Request Review *(~5 min)*

Have Bob review the pull request it just created using the GitHub MCP connection, assessing validation completeness and security, and giving a recommendation.

**Output:** PR review comment and recommendation

## Prompt Text

```text
Using the GitHub MCP connection, review the pull request you just created.

Look up my GitHub username from the authenticated token — do not guess it.

1. Read the open pull request on branch:
...
2. Review the changed code and tell me:

   - whether these three validation checks are sufficient for a banking withdrawal or if anything critical is missing

   - any security concerns with how the validation is implemented

   - whether you would approve or request changes, and why

3. Draft one inline review comment on the most important line in the change.
```

## Covers

- ✓ Pull request read directly from GitHub
- ✓ Validation completeness and security assessed
- ✓ Approve / request-changes recommendation given
- ✓ Inline review comment drafted

> **Tip:** Use this step to practice AI-assisted code review, not just generation.
